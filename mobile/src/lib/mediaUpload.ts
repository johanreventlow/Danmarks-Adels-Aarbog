...
// Deterministisk-nok, kollisionsfri sti-base under redaktørens eget upload-rum, delt af alle tre
// tiers af ÉN upload (kun til debugging/oversigt i Storage — relationen mellem tiers ligger reelt
// i DB'en). Altid .jpg: alle tiers genkodes til JPEG uanset kildeformat (se buildVariants).
...
async function resizeToTier(originalUri: string, originalWidth: number | null, tier: MediaTier, storagePath: string): Promise<ResizedVariant> {
  ...
  return {
    tier, uri: saved.uri, storagePath, mimeType: 'image/jpeg',
...
export async function buildVariants(picked: PickedImage): Promise<Record<MediaTier, ResizedVariant>> {
  const base = buildStoragePathBase();
  const [thumb, medium, large] = await Promise.all([
    resizeToTier(picked.uri, picked.width, 'thumb', `${base}-thumb.jpg`),
    resizeToTier(picked.uri, picked.width, 'medium', `${base}-medium.jpg`),
    resizeToTier(picked.uri, picked.width, 'large', `${base}-large.jpg`),
  ]);
