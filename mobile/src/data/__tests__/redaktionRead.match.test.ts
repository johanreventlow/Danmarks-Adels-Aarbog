...
describe('buildMatchPersoner — MatchFrame-input fra DB (§11)', () => {
  ...
  it('fødsel/død fra VALGT assertions date_min/date_max', () => {
  ...
  it('erhverv-fakta ignoreres; ikke-valgt assertion bruges aldrig', () => {
    const p1 = buildMatchPersoner(persons, facts, concs, assertions, extIds).find((x) => x.id === '1')!;
    expect(p1.foedsel?.date_min).not.toBe('1700-01-01');
  });

  it('kilde-medlemskab samles pr. person', () => {
  ...
  it('person uden konkluderet dato → null-intervaller', () => {
...
describe('parseIkkeSammeSomPar', () => {
  it('relation-rækker → afvisnings-par som strenge', () => {
