...
-- =============================================================
-- 2026-07-31: IDENTITY PÅ ALLE BIGINT-ID-TABELLER (replay-forudsætning, RED-8)
--
-- Problem: al id-allokering skete med `SELECT coalesce(max(id),0)+1` — en
-- læs-så-skriv-race der kolliderer ved to samtidige skrivere, og som gjorde
-- append-loads afhængige af loaderens egen seed_seq()-disciplin. Replay-laget
-- (docs/replay-source-scoped-design.md) åbner for gentagne loads og flere
-- skrivere mod samme id-rum; racen skal væk FØR den tages i brug.
--
-- Del 1 (denne blok): GENERATED BY DEFAULT AS IDENTITY på alle bigint-id-
-- tabeller + sekvens-sync til max(id)+1. BY DEFAULT (ikke ALWAYS) med vilje:
-- loaderens nid() må fortsat sætte eksplicitte id'er; den skal blot efterlade
-- sekvenserne synkede (loaderens sync_id_sequences() efter load).
-- Del 2 (nedenfor): RPC'erne omskrives fra max(id)+1 til DEFAULT-inserts.
--
-- import_korrektion og suggestion har allerede IDENTITY og springes over af
-- is_identity-guarden. Idempotent: genkørsel er ren no-op (setval kører KUN
-- sammen med ALTER, se fund 3-kommentaren nedenfor).
--
-- PROD-APPLY (Codex-review fund 4): kør atomisk og med lås-timeout —
--   psql -1 -v ON_ERROR_STOP=1 -c "SET lock_timeout='5s'" -f db-migrations.sql
-- (eller BEGIN; SET LOCAL lock_timeout='5s'; …; COMMIT; i SQL-editoren).
-- ALTER'ne tager ACCESS EXCLUSIVE kortvarigt pr. tabel; 5s-timeout gør at en
-- travl base afviser frem for at køre halvt.
-- =============================================================
DO $$
DECLARE
  t TEXT;
  ...
  FOREACH t IN ARRAY tabeller LOOP
    IF EXISTS (
      ...
      WHERE table_schema='public' AND table_name=t AND column_name='id'
        AND data_type='bigint' AND is_identity='NO'
    ) THEN
      EXECUTE format('ALTER TABLE %I ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY', t);
      -- Sekvens-sync KUN sammen med ALTER (Codex-review 2026-07-31 fund 3):
      -- ALTER holder tabel-låsen, så MAX(id) er stabil her. En ubetinget
      -- re-sync ved genkørsel ville derimod køre uden lås under live trafik
      -- og kunne gen-udlevere et id en samtidig nextval allerede har taget.
      -- Efterfølgende sync er loadernes ansvar (fail-closed, under egen lås).
      EXECUTE format(
        'SELECT setval(pg_get_serial_sequence(%L, ''id''), (SELECT coalesce(max(id),0)+1 FROM %I), false)',
        t, t);
    END IF;
  END LOOP;
END $$;
...
-- =============================================================
-- 2026-07-31: IDENTITY DEL 2 — RPC'erne fra max(id)+1 til DEFAULT-inserts
--
-- Nyeste definition af hver berørt funktion (26 stk.), mekanisk omskrevet:
-- id udeladt af kolonneliste/VALUES (sekvensen allokerer), RETURNING id INTO
-- hvor id'et bruges videre. Grants/ACL bevares af CREATE OR REPLACE.
-- red_set_story_kilder mistede samtidig sin v_next-løbenummer-variabel.
-- Loaderens nid() består (BY DEFAULT tillader eksplicitte id'er) men SKAL
-- efterlade sekvenserne synkede — load_daa.R kører sync_id_sequences()
-- som sidste skridt før commit.
-- =============================================================
CREATE OR REPLACE FUNCTION _ensure_foraeldrefamilie_redaktionel(p_barn bigint, p_family bigint)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_slot bigint; v_assert bigint;
BEGIN
  ...
  SELECT id INTO v_slot FROM fact
    WHERE subjekt_type='person' AND subjekt_id=p_barn AND faktatype='forældrefamilie' LIMIT 1;
  IF v_slot IS NULL THEN
    INSERT INTO fact(subjekt_type, subjekt_id, faktatype)
      VALUES ('person', p_barn, 'forældrefamilie') RETURNING id INTO v_slot;
  END IF;
  -- Find KUN en redaktionel (source-løs) assertion for familien — kapr aldrig en source-bunden
  -- rival-/adjudikeret påstand (review 30/Codex #5). Ordnet for determinisme.
  SELECT a.id INTO v_assert FROM assertion a
    ...
    ORDER BY a.id LIMIT 1;
  IF v_assert IS NULL THEN
    INSERT INTO assertion(target_type, target_id, vaerdi_tekst, objekt_type, objekt_id, uforanderlig)
      VALUES ('fact', v_slot, 'barn', 'family', p_family, true) RETURNING id INTO v_assert;
    INSERT INTO citation(assertion_id, source_id, citat_tekst, kvalitet)
      VALUES (v_assert, NULL, '(kilde mangler: strukturel projektion)', 'sekundær');
  END IF;
  INSERT INTO conclusion(target_type, target_id, valgt_assertion_id, status, blaastemplet_af, blaastemplet_naar)
    VALUES ('fact', v_slot, v_assert, 'afklaret', 'Redaktør (strukturel)', current_date)
    ON CONFLICT (target_type, target_id)
    DO UPDATE SET valgt_assertion_id=excluded.valgt_assertion_id, status='afklaret', blaastemplet_naar=current_date;
END $$;
...
BEGIN
  ...
  v_navn  := coalesce(v_navn, 'ukendt');
  v_rolle := coalesce(v_rolle, 'medlem');
    INSERT INTO change_set(actor_id, actor_navn, actor_rolle, operation, summary,
                         subjekt_type, subjekt_id, subjekt_synlighed)
    VALUES (v_uid, v_navn, v_rolle, p_operation, p_summary,
            p_subjekt_type, p_subjekt_id, _subjekt_synlighed(p_subjekt_type, p_subjekt_id)) RETURNING id INTO v_id;
  PERFORM set_config('app.change_set_id', v_id::text, true);
  PERFORM set_config('app.change_seq', '0', true);
  RETURN v_id;
END $$;
...
BEGIN
  ...
  INSERT INTO change_event(change_set_id, seq, tabel, row_pk, op, foer, efter)
  VALUES (v_cs::bigint, v_seq, TG_TABLE_NAME,
          _row_pk(TG_TABLE_NAME, coalesce(v_new, v_old)),
          TG_OP, v_foer, v_efter);
  RETURN NULL;
END $$;
...
BEGIN
  ...
  INSERT INTO assertion(target_type, target_id, vaerdi_tekst, date_min, date_max,
                        date_qualifier, date_raw, calendar, uforanderlig)
    VALUES (v_old.target_type, v_old.target_id,
            p_vaerdi, v_old.date_min, v_old.date_max, v_old.date_qualifier,
            coalesce(p_date_raw, v_old.date_raw), v_old.calendar, false)
    RETURNING id INTO v_new;

  INSERT INTO citation(assertion_id, source_id, citat_tekst)
    VALUES (v_new, NULL,
            coalesce(p_kilde_fritekst, '(kilde mangler)'))
    RETURNING id INTO v_cit;
...
BEGIN
  ...
  PERFORM set_config('app.change_set_id','',true);  -- undgå at inverse-DML re-logges
    ...
    VALUES (auth.uid(),
            ...
            current_rolle(), 'fortryd',
            format('Fortrød: %s', coalesce(v_orig.summary,'(uden tekst)')),
            v_orig.subjekt_type, v_orig.subjekt_id, v_orig.subjekt_synlighed, p_change_set_id) RETURNING id INTO v_new_cs;
...
BEGIN
  ...
  INSERT INTO relation(subjekt_type, subjekt_id, objekt_type, objekt_id, rolle)
    VALUES ('person', v_lo, 'person', v_hi, 'ikke_samme_som')
    RETURNING id INTO v_rel;
  INSERT INTO assertion(target_type, target_id, vaerdi_tekst)
    VALUES ('relation', v_rel, 'ikke_samme_som')
    RETURNING id INTO v_ass;
  INSERT INTO conclusion(target_type, target_id, valgt_assertion_id, status, blaastemplet_af)
    VALUES ('relation', v_rel, v_ass, 'afklaret',
            'redaktionel identitets-afvisning');
  RETURN v_rel;
END $$;
...
BEGIN
  ...
  PERFORM begin_change_set('red_opret_estate', format('Oprettede ejendom %s', p_navn), NULL, NULL);
  IF nullif(btrim(p_navn),'') IS NULL THEN RAISE EXCEPTION 'Navn er påkrævet'; END IF;
    INSERT INTO estate(navn, slags, sted_id) VALUES (p_navn, p_slags, p_sted_id) RETURNING id INTO v_id;
  RETURN v_id;
END $$;
...
BEGIN
  ...
  PERFORM begin_change_set('red_opret_fakta', format('Oprettede %s på %s/%s', p_faktatype, p_subjekt_type, p_subjekt_id), p_subjekt_type, p_subjekt_id);
  INSERT INTO fact(subjekt_type, subjekt_id, faktatype)
    VALUES (p_subjekt_type, p_subjekt_id, p_faktatype)
    RETURNING id INTO v_fact;
  INSERT INTO assertion(target_type, target_id, vaerdi_tekst,
                        date_min, date_max, date_qualifier, date_raw, uforanderlig)
    VALUES ('fact', v_fact, p_vaerdi,
            p_date_min, p_date_max, p_date_qualifier, p_date_raw, false)
    RETURNING id INTO v_assert;
  INSERT INTO citation(assertion_id, source_id, citat_tekst)
    VALUES (v_assert, NULL,
            coalesce(p_kilde_fritekst,'(kilde mangler)'))
    RETURNING id INTO v_cit;
  INSERT INTO conclusion(target_type, target_id, valgt_assertion_id, status, blaastemplet_af, blaastemplet_naar)
    VALUES ('fact', v_fact, v_assert,
            'afklaret', 'Redaktør', current_date);
  RETURN jsonb_build_object('fact_id', v_fact, 'assertion_id', v_assert, 'citation_id', v_cit);
END $$;
...
BEGIN
  ...
  PERFORM begin_change_set('red_opret_kilde', format('Oprettede kilde %s', p_titel), NULL, NULL);
  IF nullif(btrim(p_titel),'') IS NULL THEN RAISE EXCEPTION 'Titel er påkrævet'; END IF;
    INSERT INTO source(slags, titel, udgave, aar, ekstern) VALUES (p_slags, p_titel, p_udgave, p_aar, p_ekstern) RETURNING id INTO v_id;
  RETURN v_id;
END $$;
...
BEGIN
  ...
  IF p_sha256 IS NOT NULL AND EXISTS (SELECT 1 FROM media WHERE sha256 = p_sha256) THEN
    RAISE EXCEPTION 'Medie med samme indhold findes allerede (sha256=%). Genbrug den eksisterende media-række via red_relation.', p_sha256;
  END IF;
    INSERT INTO media(slags, titel, kunstner, datering, bucket, storage_path,
                    mime_type, byte_size, bredde, hoejde, sha256, original_filnavn,
                    upload_status, rettigheder_status, maa_publiceres)
    VALUES (p_slags, p_titel, p_kunstner, p_datering, coalesce(p_bucket,'media'), p_storage_path,
            p_mime, p_byte_size, p_bredde, p_hoejde, p_sha256, p_original_filnavn,
            'kladde', coalesce(p_rettigheder_status,'ukendt'), coalesce(p_maa_publiceres,false)) RETURNING id INTO v_id;
  RETURN v_id;
END $$;
...
BEGIN
  ...
  PERFORM begin_change_set('red_opret_organisation', format('Oprettede organisation %s', p_navn), NULL, NULL);
  IF nullif(btrim(p_navn),'') IS NULL THEN RAISE EXCEPTION 'Navn er påkrævet'; END IF;
    INSERT INTO organisation(navn, slags) VALUES (p_navn, p_slags) RETURNING id INTO v_id;
  RETURN v_id;
END $$;
...
BEGIN
  ...
  IF p_koen IS NOT NULL AND p_koen NOT IN ('mand','kvinde','ukendt')
    THEN RAISE EXCEPTION 'Ugyldigt køn %', p_koen; END IF;
    INSERT INTO person(levende, privat, koen) VALUES (p_levende, true, p_koen) RETURNING id INTO v_id;
  PERFORM red_upsert_fakta('person', v_id, 'navn', p_navn);
  IF nullif(btrim(p_foedt_raw),'') IS NOT NULL THEN
    PERFORM red_upsert_fakta('person', v_id, 'fødsel', p_foedt_raw, p_date_raw => p_foedt_raw);
  END IF;
...
BEGIN
  ...
  PERFORM begin_change_set('red_opret_story',
    format('Oprettede story om %s %s', p_subjekt_type, p_subjekt_id), p_subjekt_type, p_subjekt_id);
  INSERT INTO story (subjekt_type, subjekt_id, haendelse_id, fact_id, relation_id,
                     historical_event_id, titel, tekst, date_min, date_max, date_qualifier,
                     date_raw, privat)
  VALUES (p_subjekt_type, p_subjekt_id,
          p_haendelse_id, p_fact_id, p_relation_id, p_historical_event_id,
          p_titel, p_tekst, p_date_min, p_date_max, p_date_qualifier, p_date_raw, p_privat)
  RETURNING id INTO v_id;
  RETURN v_id;
...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM person WHERE id=p_partner_b) THEN RAISE EXCEPTION 'Person % findes ikke', p_partner_b; END IF;
  IF p_type NOT IN ('vielse','partnerskab','ugift union') THEN RAISE EXCEPTION 'Ugyldig union-type %', p_type; END IF;
  INSERT INTO family(type) VALUES (p_type) RETURNING id INTO v_fam;
  INSERT INTO family_member(family_id, person_id, rolle, ordinal, konfidens) VALUES (v_fam, p_partner_a, 'partner', p_ordinal, NULL);
  INSERT INTO family_member(family_id, person_id, rolle, ordinal, konfidens) VALUES (v_fam, p_partner_b, 'partner', p_ordinal, NULL);
  RETURN v_fam;
END $$;
...
BEGIN
  ...
  IF p_tier NOT IN ('thumb','medium') THEN
    RAISE EXCEPTION '''%'' er ikke en gyldig variant-tier. ''large'' er media-rækkens egen storage_path, ikke en variant-række.', p_tier;
  END IF;
  INSERT INTO media_variant(media_id, tier, storage_path, mime_type, byte_size, bredde, hoejde)
    VALUES (p_media_id, p_tier, p_storage_path, p_mime, p_byte_size, p_bredde, p_hoejde)
    ON CONFLICT (media_id, tier) DO UPDATE
      SET storage_path = excluded.storage_path, mime_type = excluded.mime_type,
          byte_size = excluded.byte_size, bredde = excluded.bredde, hoejde = excluded.hoejde
    RETURNING id INTO v_id;
...
BEGIN
  ...
  PERFORM begin_change_set('red_relation', format('Relation %s: %s/%s → %s/%s', p_rolle, p_subjekt_type, p_subjekt_id, p_objekt_type, p_objekt_id), p_subjekt_type, p_subjekt_id);
  BEGIN
    INSERT INTO relation(subjekt_type, subjekt_id, objekt_type, objekt_id, rolle, periode_raw)
      VALUES (p_subjekt_type, p_subjekt_id, p_objekt_type, p_objekt_id, p_rolle, p_periode_raw)
      RETURNING id INTO v_id;
  EXCEPTION WHEN unique_violation THEN
    DECLARE v_constraint_name text;
    BEGIN
...
BEGIN
  ...
  -- Triggeren validerer G0/G3/G4/G5 på denne INSERT.
  INSERT INTO relation(subjekt_type, subjekt_id, objekt_type, objekt_id, rolle)
    VALUES ('person', p_alias_id, 'person', p_objekt_id, 'samme_som')
    RETURNING id INTO v_rel;
  INSERT INTO assertion(target_type, target_id, vaerdi_tekst)
    VALUES ('relation', v_rel, 'samme_som')
    RETURNING id INTO v_ass;
  INSERT INTO conclusion(target_type, target_id, valgt_assertion_id, status, blaastemplet_af)
    VALUES ('relation', v_rel, v_ass, 'afklaret',
            'redaktionel identitetssammenkædning');
...
BEGIN
  ...
  PERFORM begin_change_set('red_set_feed_pin',
    format('Satte %s på kort %s', p_handling, p_kort_noegle), NULL, NULL);
  INSERT INTO feed_pin (kort_noegle, handling)
  VALUES (p_kort_noegle, p_handling)
  ON CONFLICT (kort_noegle) DO UPDATE
    SET handling=excluded.handling, oprettet_af=auth.uid(), oprettet_naar=now();
END $$;
...
BEGIN
  ...
  -- delvist) ville ellers give silent no-op (0 rækker) + falsk success.
  INSERT INTO conclusion(target_type, target_id, valgt_assertion_id, status, blaastemplet_af, blaastemplet_naar)
    VALUES (v_target_type, v_target_id, p_assertion_id,
            'afklaret', 'Redaktør', current_date)
    ON CONFLICT (target_type, target_id)
    DO UPDATE SET valgt_assertion_id = excluded.valgt_assertion_id, blaastemplet_naar = current_date;
END $$;
...
BEGIN
  ...
  IF v_fact IS NULL THEN
    INSERT INTO fact(subjekt_type, subjekt_id, faktatype)
      VALUES ('person', p_barn_id, 'forældrefamilie')
      RETURNING id INTO v_fact;
  END IF;

  INSERT INTO assertion(target_type, target_id, vaerdi_tekst, objekt_type, objekt_id, uforanderlig)
    VALUES ('fact', v_fact, 'barn', 'family', p_family_id, true)
    RETURNING id INTO v_assert;
  INSERT INTO citation(assertion_id, source_id, side, citat_tekst, kvalitet)
    VALUES (v_assert, p_source_id, p_side,
            coalesce(p_citat, p_kilde_fritekst, '(kilde mangler)'),
            CASE WHEN p_source_id IS NOT NULL THEN 'primær' ELSE 'sekundær' END)  -- kildeløs redaktionel påstand er ikke primærkilde
    RETURNING id INTO v_cit;
  ...
  ELSE
    ...
    IF v_barn_family IS NOT DISTINCT FROM p_family_id THEN
      INSERT INTO conclusion(target_type, target_id, valgt_assertion_id, status, blaastemplet_af, blaastemplet_naar)
        VALUES ('fact', v_fact, v_assert, 'afklaret',
                'Redaktør (korroboration)', current_date);
    END IF;
    -- ellers: påstanden står som ren kandidat; adjudicér med red_vaelg_foraeldre.
  END IF;
...
BEGIN
  ...
  INSERT INTO assertion(target_type, target_id, vaerdi_tekst,
                        date_min, date_max, date_qualifier, date_raw, uforanderlig)
    VALUES ('fact', p_fact_id, p_vaerdi,
            p_date_min, p_date_max, p_date_qualifier, p_date_raw, false)
    RETURNING id INTO v_assert;
  INSERT INTO citation(assertion_id, source_id, citat_tekst)
    VALUES (v_assert, NULL,
...
BEGIN
  ...
  IF v_id IS NOT NULL THEN RETURN v_id; END IF;
  INSERT INTO relation(subjekt_type, subjekt_id, objekt_type, objekt_id, rolle, periode_raw)
    VALUES ('person', p_subjekt_id, p_objekt_type, p_objekt_id, p_rolle, p_periode_raw)
    RETURNING id INTO v_id;
  RETURN v_id;
END $$;
...
BEGIN
  ...
  SELECT id INTO v_id FROM narrative
    ...
    ORDER BY id LIMIT 1;
  IF v_id IS NULL THEN
    INSERT INTO narrative(subjekt_type, subjekt_id, source_id, tekst, side, privat)
      VALUES (p_subjekt_type, p_subjekt_id, p_source_id, p_tekst, p_side, p_privat)
      RETURNING id INTO v_id;
  ELSE
    UPDATE narrative SET tekst=p_tekst, privat=p_privat, side=COALESCE(p_side, side) WHERE id=v_id;
  END IF;
...
BEGIN
  ...
  -- slottet allerede peger på til-familien og ikke dobbelt-nedskriver).
  INSERT INTO conclusion(target_type, target_id, valgt_assertion_id, status, blaastemplet_af, blaastemplet_naar)
    VALUES ('fact', v_fact, p_assertion_id, 'afklaret', 'Redaktør', current_date)
    ON CONFLICT (target_type, target_id)
    DO UPDATE SET valgt_assertion_id=excluded.valgt_assertion_id, status='afklaret', blaastemplet_naar=current_date;
...
CREATE OR REPLACE FUNCTION red_set_story_kilder(p_story_id bigint, p_kilder jsonb)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_stype text; v_sid bigint; v_k jsonb;
BEGIN
  IF current_rolle() <> 'redaktion' THEN RAISE EXCEPTION 'Kun redaktion'; END IF;
  IF p_kilder IS NULL OR jsonb_typeof(p_kilder) <> 'array' THEN
    RAISE EXCEPTION 'p_kilder skal være et jsonb-array af {source_id, side?}';
  ...
  FOR v_k IN SELECT * FROM jsonb_array_elements(p_kilder) LOOP
    IF v_k->>'source_id' IS NULL
      ...
      RAISE EXCEPTION 'Source % findes ikke', v_k->>'source_id';
    END IF;
    INSERT INTO story_kilde (story_id, source_id, side)
...
-- =====================================================================
-- EVIDENSBASERET GENIMPORT FASE 2 / TASK 4 — PRIVAT APPEND-ONLY KILDELAG
-- Idempotent opgraderingssti. En delvist oprettet tabel er ikke understøttet:
-- migrationen skal enten være kørt samlet eller rulles tilbage af transaktionen.
-- =====================================================================
CREATE SCHEMA IF NOT EXISTS private;
REVOKE ALL ON SCHEMA private FROM PUBLIC,anon,authenticated;
...
CREATE TABLE IF NOT EXISTS private.extraction_run (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS private.source_rendition (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  source_id bigint NOT NULL REFERENCES public.source(id),
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(source_id,rendition_key), UNIQUE(id,source_id)
);
CREATE INDEX IF NOT EXISTS source_rendition_content_idx
  ON private.source_rendition(source_id,content_sha256);
CREATE TABLE IF NOT EXISTS private.source_record (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  source_id bigint NOT NULL REFERENCES public.source(id),
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(source_id,record_key), UNIQUE(id,source_id),
  FOREIGN KEY(created_run_id,source_id) REFERENCES private.extraction_run(id,source_id)
);
CREATE TABLE IF NOT EXISTS private.source_record_occurrence (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS private.source_record_anchor_event (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  decision_status text NOT NULL CHECK(decision_status IN ('proposed','accepted','rejected')),
  ...
  CHECK(decision_status='proposed' OR
        (nullif(btrim(decided_by_name),'') IS NOT NULL AND evidence<>'{}'::jsonb))
);
CREATE TABLE IF NOT EXISTS private.source_record_revision_event (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  relation_kind text NOT NULL CHECK(relation_kind IN ('split_into','merged_from','replaced_by')),
  decision_status text NOT NULL CHECK(decision_status IN ('proposed','accepted','rejected')),
  evidence jsonb NOT NULL CHECK(jsonb_typeof(evidence)='object'),
  version integer NOT NULL CHECK(version>=1),
  decided_by uuid,
...
CREATE TABLE IF NOT EXISTS private.source_observation (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  occurrence_id uuid NOT NULL REFERENCES private.source_record_occurrence(id),
  ...
  quality_status text NOT NULL CHECK(quality_status IN
    ('clear','ocr_uncertain','truncated','structurally_uncertain','illegible')),
  extraction_method text NOT NULL CHECK(btrim(extraction_method)<>''),
  extraction_run_id uuid NOT NULL REFERENCES private.extraction_run(id),
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS private.source_observation_text (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS private.source_mention (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  observation_id uuid NOT NULL REFERENCES private.source_observation(id),
...
CREATE TABLE IF NOT EXISTS private.source_persona (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  source_id bigint NOT NULL REFERENCES public.source(id),
  persona_key text NOT NULL CHECK(btrim(persona_key)<>''),
  created_run_id uuid NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(source_id,persona_key), UNIQUE(id,source_id),
  FOREIGN KEY(created_run_id,source_id) REFERENCES private.extraction_run(id,source_id)
);
CREATE TABLE IF NOT EXISTS private.source_persona_mention (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY(persona_id,mention_id), UNIQUE(persona_id,ordinal)
);

CREATE OR REPLACE VIEW private.source_record_anchor_current
WITH (security_invoker=true) AS
...
CREATE OR REPLACE VIEW private.source_record_revision_current
WITH (security_invoker=true) AS
...
CREATE OR REPLACE VIEW private.source_observation_text_current
WITH (security_invoker=true) AS
...
CREATE OR REPLACE FUNCTION private.reject_evidence_mutation()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog AS $$
BEGIN
  RAISE EXCEPTION 'EVIDENCE_APPEND_ONLY: %.% kan ikke %',TG_TABLE_SCHEMA,TG_TABLE_NAME,TG_OP;
END $$;

CREATE OR REPLACE FUNCTION private.validate_evidence_scope()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,public,private AS $$
DECLARE v_source_a bigint; v_source_b bigint; v_text text;
BEGIN
  ...
  ELSIF TG_TABLE_NAME='source_observation' THEN
    SELECT r.source_id INTO v_source_a FROM private.source_record_occurrence ro
      JOIN private.source_rendition r ON r.id=ro.rendition_id WHERE ro.id=NEW.occurrence_id;
    SELECT source_id INTO v_source_b FROM private.extraction_run WHERE id=NEW.extraction_run_id;
  ...
  ELSIF TG_TABLE_NAME='source_observation_text' THEN
    SELECT r.source_id INTO v_source_a FROM private.source_observation o
    ...
    IF (NEW.version=1 AND NEW.supersedes_id IS NOT NULL) OR
       (NEW.version>1 AND NOT EXISTS(SELECT 1 FROM private.source_observation_text p
          WHERE p.id=NEW.supersedes_id AND p.observation_id=NEW.observation_id
            AND p.rendition_id=NEW.rendition_id AND p.version=NEW.version-1)) THEN
      RAISE EXCEPTION 'EVIDENCE_TEXT_VERSION_CONFLICT';
    END IF;
  ELSIF TG_TABLE_NAME='source_mention' THEN
    SELECT o.verbatim_text,r.source_id INTO v_text,v_source_a FROM private.source_observation o
      JOIN private.source_record_occurrence ro ON ro.id=o.occurrence_id
      JOIN private.source_rendition r ON r.id=ro.rendition_id WHERE o.id=NEW.observation_id;
    SELECT source_id INTO v_source_b FROM private.extraction_run WHERE id=NEW.created_run_id;
    IF NEW.char_to>length(v_text) OR
       substr(v_text,NEW.char_from+1,NEW.char_to-NEW.char_from)<>NEW.verbatim_text THEN
  ...
  ELSIF TG_TABLE_NAME='source_persona_mention' THEN
    ...
    SELECT r.source_id INTO v_source_b FROM private.source_mention m
      JOIN private.source_observation o ON o.id=m.observation_id
      JOIN private.source_record_occurrence ro ON ro.id=o.occurrence_id
      JOIN private.source_rendition r ON r.id=ro.rendition_id WHERE m.id=NEW.mention_id;
  END IF;
...
CREATE OR REPLACE FUNCTION private.validate_source_record_anchor_event()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,public,private AS $$
DECLARE v_occurrence_source bigint; v_record_source bigint; v_expected_version integer;
BEGIN
  IF NEW.decision_status<>'proposed' AND
     (NEW.decided_by IS NULL OR nullif(btrim(NEW.decided_by_name),'') IS NULL
  ...
  SELECT r.source_id INTO v_occurrence_source FROM private.source_record_occurrence o
    JOIN private.source_rendition r ON r.id=o.rendition_id WHERE o.id=NEW.occurrence_id;
  ...
  IF NEW.decision_status='accepted' AND EXISTS(
    ...
    WHERE c.occurrence_id=NEW.occurrence_id AND c.decision_status='accepted') THEN
    RAISE EXCEPTION 'EVIDENCE_ANCHOR_CONFLICT: occurrence har allerede en accepted anchor';
  END IF;
  RETURN NEW;
END $$;

CREATE OR REPLACE FUNCTION private.validate_source_record_revision_event()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,public,private AS $$
DECLARE v_predecessor_source bigint; v_successor_source bigint; v_expected_version integer;
BEGIN
  IF NEW.decision_status<>'proposed' AND
     (NEW.decided_by IS NULL OR nullif(btrim(NEW.decided_by_name),'') IS NULL
      OR NEW.evidence='{}'::jsonb) THEN
  ...
  PERFORM id FROM private.source_record
    WHERE id IN(NEW.predecessor_record_id,NEW.successor_record_id) ORDER BY id FOR UPDATE;
  SELECT source_id INTO v_predecessor_source FROM private.source_record
    WHERE id=NEW.predecessor_record_id;
  SELECT source_id INTO v_successor_source FROM private.source_record
    WHERE id=NEW.successor_record_id;
  ...
  SELECT coalesce(max(version),0)+1 INTO v_expected_version
    ...
    WHERE predecessor_record_id=NEW.predecessor_record_id
      AND successor_record_id=NEW.successor_record_id AND relation_kind=NEW.relation_kind;
  IF NEW.version<>v_expected_version THEN
    RAISE EXCEPTION 'EVIDENCE_EVENT_VERSION_CONFLICT: expected %, got %',v_expected_version,NEW.version;
  END IF;
  RETURN NEW;
...
BEGIN
  FOREACH v_table IN ARRAY ARRAY[
    'source_record_occurrence','source_observation','source_observation_text',
    'source_mention','source_persona_mention'
  ] LOOP
    v_trigger='trg_'||v_table||'_scope';
    EXECUTE format('DROP TRIGGER IF EXISTS %I ON private.%I',v_trigger,v_table);
    EXECUTE format('CREATE TRIGGER %I BEFORE INSERT ON private.%I FOR EACH ROW EXECUTE FUNCTION private.validate_evidence_scope()',v_trigger,v_table);
  END LOOP;
  DROP TRIGGER IF EXISTS trg_source_record_anchor_event_validate ON private.source_record_anchor_event;
  CREATE TRIGGER trg_source_record_anchor_event_validate BEFORE INSERT
    ON private.source_record_anchor_event FOR EACH ROW
    EXECUTE FUNCTION private.validate_source_record_anchor_event();
  DROP TRIGGER IF EXISTS trg_source_record_revision_event_validate ON private.source_record_revision_event;
  CREATE TRIGGER trg_source_record_revision_event_validate BEFORE INSERT
    ON private.source_record_revision_event FOR EACH ROW
    EXECUTE FUNCTION private.validate_source_record_revision_event();
  FOREACH v_table IN ARRAY ARRAY[
...
CREATE TABLE IF NOT EXISTS private.interpretation (
  ...
  CONSTRAINT interpretation_decision_audit_check CHECK(status='proposed' OR
  ...
  CONSTRAINT interpretation_proposal_actor_check CHECK(status<>'proposed' OR
    (decided_by IS NULL AND decided_by_name IS NULL AND decided_at IS NULL))
);
...
CREATE TABLE IF NOT EXISTS private.source_persona_identity (
  ...
  CONSTRAINT source_persona_identity_decision_audit_check CHECK(decision_status='proposed' OR
    (decided_by IS NOT NULL AND nullif(btrim(decided_by_name),'') IS NOT NULL
     AND decided_at IS NOT NULL AND evidence<>'{}'::jsonb)),
  CONSTRAINT source_persona_identity_proposal_actor_check CHECK(decision_status<>'proposed' OR
    (decided_by IS NULL AND decided_by_name IS NULL AND decided_at IS NULL))
);
CREATE TABLE IF NOT EXISTS private.source_persona_identity_event (
  ...
  CHECK(decision_status<>'accepted' OR canonical_person_id IS NOT NULL),
  CHECK(decision_status<>'unresolved' OR canonical_person_id IS NULL),
  CONSTRAINT source_persona_identity_event_decision_audit_check CHECK(decision_status='proposed' OR
    (decided_by IS NOT NULL AND nullif(btrim(decided_by_name),'') IS NOT NULL
     AND decided_at IS NOT NULL AND evidence<>'{}'::jsonb)),
  CONSTRAINT source_persona_identity_event_proposal_actor_check CHECK(decision_status<>'proposed' OR
    (decided_by IS NULL AND decided_by_name IS NULL AND decided_at IS NULL))
);

-- Opgrader også en database, der nåede den første Task 5-version: erstat de
-- automatisk navngivne actor-CHECKs med den endelige proposal/decision-kontrakt.
DO $$
DECLARE
  v_table text;
  v_constraint record;
BEGIN
  FOREACH v_table IN ARRAY ARRAY[
    'interpretation','source_persona_identity','source_persona_identity_event'
  ] LOOP
    FOR v_constraint IN
      SELECT c.conname
       ...
       WHERE n.nspname='private' AND t.relname=v_table AND c.contype='c'
         AND pg_catalog.pg_get_constraintdef(c.oid) LIKE '%decided_by%'
         AND c.conname NOT IN (
           'interpretation_decision_audit_check','interpretation_proposal_actor_check',
           'source_persona_identity_decision_audit_check',
           'source_persona_identity_proposal_actor_check',
           'source_persona_identity_event_decision_audit_check',
           'source_persona_identity_event_proposal_actor_check'
         )
    LOOP
      EXECUTE format('ALTER TABLE private.%I DROP CONSTRAINT %I',v_table,v_constraint.conname);
    END LOOP;
  END LOOP;

  IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conrelid='private.interpretation'::regclass
      AND conname='interpretation_decision_audit_check') THEN
    ALTER TABLE private.interpretation ADD CONSTRAINT interpretation_decision_audit_check
  ...
  IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conrelid='private.interpretation'::regclass
      AND conname='interpretation_proposal_actor_check') THEN
    ALTER TABLE private.interpretation ADD CONSTRAINT interpretation_proposal_actor_check
      CHECK(status<>'proposed' OR
        (decided_by IS NULL AND decided_by_name IS NULL AND decided_at IS NULL));
  END IF;
  IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conrelid='private.source_persona_identity'::regclass
      AND conname='source_persona_identity_decision_audit_check') THEN
    ALTER TABLE private.source_persona_identity
      ADD CONSTRAINT source_persona_identity_decision_audit_check
      CHECK(decision_status='proposed' OR
        (decided_by IS NOT NULL AND nullif(btrim(decided_by_name),'') IS NOT NULL
         AND decided_at IS NOT NULL AND evidence<>'{}'::jsonb));
  END IF;
  IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conrelid='private.source_persona_identity'::regclass
      AND conname='source_persona_identity_proposal_actor_check') THEN
    ALTER TABLE private.source_persona_identity
      ADD CONSTRAINT source_persona_identity_proposal_actor_check
      CHECK(decision_status<>'proposed' OR
        (decided_by IS NULL AND decided_by_name IS NULL AND decided_at IS NULL));
  END IF;
  IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conrelid='private.source_persona_identity_event'::regclass
      AND conname='source_persona_identity_event_decision_audit_check') THEN
    ALTER TABLE private.source_persona_identity_event
      ADD CONSTRAINT source_persona_identity_event_decision_audit_check
      CHECK(decision_status='proposed' OR
        (decided_by IS NOT NULL AND nullif(btrim(decided_by_name),'') IS NOT NULL
         AND decided_at IS NOT NULL AND evidence<>'{}'::jsonb));
  END IF;
  IF NOT EXISTS(SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conrelid='private.source_persona_identity_event'::regclass
      AND conname='source_persona_identity_event_proposal_actor_check') THEN
    ALTER TABLE private.source_persona_identity_event
      ADD CONSTRAINT source_persona_identity_event_proposal_actor_check
      CHECK(decision_status<>'proposed' OR
        (decided_by IS NULL AND decided_by_name IS NULL AND decided_at IS NULL));
  END IF;
END $$;
...
BEGIN
  ...
  RETURN NEW;
END $$;
CREATE OR REPLACE FUNCTION private.validate_source_persona_identity_state()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,private AS $$
BEGIN
  IF NEW.decision_status<>'proposed' AND current_user<>(
    SELECT pg_catalog.pg_get_userbyid(c.relowner)
...
BEGIN
  ...
  SELECT EXISTS(SELECT 1 FROM private.source_persona_identity
                 WHERE source_persona_id=v_persona_id)
    INTO v_state_exists;
  ...
  IF NOT EXISTS(SELECT 1 FROM private.source_persona_identity
    WHERE source_persona_id=v_persona_id AND version=v_latest_version) THEN
  ...
  IF NOT EXISTS(
    ...
    WHERE i.source_persona_id=v_persona_id
      ...
      AND i.decision_status=e.decision_status AND i.evidence=e.evidence
...
BEGIN
  ...
  DROP TRIGGER IF EXISTS trg_source_persona_identity_state_validate
    ON private.source_persona_identity;
  CREATE TRIGGER trg_source_persona_identity_state_validate BEFORE INSERT OR UPDATE
    ON private.source_persona_identity FOR EACH ROW
    EXECUTE FUNCTION private.validate_source_persona_identity_state();
  DROP TRIGGER IF EXISTS identity_state_has_event ON private.source_persona_identity;
  CREATE CONSTRAINT TRIGGER identity_state_has_event AFTER INSERT OR UPDATE OR DELETE
...
-- =====================================================================
-- EVIDENSBASERET GENIMPORT FASE 3 / TASK 6 — SLÆGT OG KILDESCHEMES
-- =====================================================================
CREATE TABLE IF NOT EXISTS slaegt (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS lineage_scheme (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS lineage_scheme_slaegt_kind_no_source_uidx
  ON lineage_scheme(slaegt_id,kind) WHERE source_id IS NULL;
CREATE TABLE IF NOT EXISTS lineage_scheme_entry (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(scheme_id,code), UNIQUE(scheme_id,sort_order)
);
CREATE TABLE IF NOT EXISTS lineage_scheme_entry_lineage (
...
CREATE TABLE IF NOT EXISTS person_slaegt_membership (
...
CREATE TABLE IF NOT EXISTS person_lineage_membership (
  person_id bigint NOT NULL REFERENCES person(id),
  lineage_id bigint NOT NULL REFERENCES lineage(id),
...
-- Eksisterende lineages bliver knyttet til en generel slægt uden at bruge
-- bogens linje-kode som nøgle. Manglende historisk navn forbliver eksplicit
-- "Uafklaret slægt" frem for at gætte et efternavn.
INSERT INTO slaegt(navn,sorteringsnavn,slug,status)
SELECT n,n,slug,'historical'
FROM (
  SELECT DISTINCT coalesce(lineage_effective_slaegtsnavn(id),slaegtsnavn,'Uafklaret slægt') AS n,
    regexp_replace(lower(coalesce(lineage_effective_slaegtsnavn(id),slaegtsnavn,'uafklaret-slaegt')),
      '[^a-z0-9]+','-','g') AS slug
  FROM lineage
) names
WHERE slug<>''
ON CONFLICT(slug) DO NOTHING;
UPDATE lineage l SET
  slaegt_id=s.id,
  canonical_label=coalesce(l.canonical_label,l.navn)
FROM slaegt s
WHERE l.slaegt_id IS NULL
  AND s.slug=regexp_replace(lower(coalesce(lineage_effective_slaegtsnavn(l.id),l.slaegtsnavn,'uafklaret-slaegt')),
    '[^a-z0-9]+','-','g');

INSERT INTO lineage_scheme(slaegt_id,source_id,kind,label)
SELECT DISTINCT l.slaegt_id,l.source_id,'stamtavle',
  coalesce(nullif(btrim(s.titel),''),'Kildestamtavle')
FROM lineage l LEFT JOIN source s ON s.id=l.source_id
WHERE l.slaegt_id IS NOT NULL AND l.source_id IS NOT NULL AND l.kode IS NOT NULL
ON CONFLICT(slaegt_id,source_id,kind) DO NOTHING;
INSERT INTO lineage_scheme_entry(scheme_id,code,label,sort_order)
SELECT scheme.id,l.kode,coalesce(l.canonical_label,l.navn),row_number() OVER (
  PARTITION BY scheme.id ORDER BY l.kode
)::integer
FROM lineage l
JOIN lineage_scheme scheme ON scheme.slaegt_id=l.slaegt_id
  AND scheme.source_id=l.source_id AND scheme.kind='stamtavle'
WHERE l.kode IS NOT NULL
ON CONFLICT(scheme_id,code) DO NOTHING;
INSERT INTO lineage_scheme_entry_lineage(entry_id,lineage_id,relation_kind)
SELECT entry.id,l.id,'canonical'
FROM lineage l
JOIN lineage_scheme scheme ON scheme.slaegt_id=l.slaegt_id
  AND scheme.source_id=l.source_id AND scheme.kind='stamtavle'
JOIN lineage_scheme_entry entry ON entry.scheme_id=scheme.id AND entry.code=l.kode
WHERE l.kode IS NOT NULL
ON CONFLICT DO NOTHING;

-- Præsensnummerering er en selvstændig kildescheme pr. slægt. Det gamle globale
-- lineage.presens_kode bliver stående som overgangsprojektion, indtil både web og
-- mobil læser dette scope-bevidste mappinglag.
INSERT INTO lineage_scheme(slaegt_id,source_id,kind,label)
SELECT DISTINCT l.slaegt_id,NULL::bigint,'presensliste','Præsensliste'
FROM lineage l
WHERE l.slaegt_id IS NOT NULL AND l.presens_kode IS NOT NULL
ON CONFLICT(slaegt_id,kind) WHERE source_id IS NULL DO NOTHING;
INSERT INTO lineage_scheme_entry(scheme_id,code,label,sort_order)
SELECT scheme.id,l.presens_kode,coalesce(l.canonical_label,l.navn),row_number() OVER (
  PARTITION BY scheme.id ORDER BY l.presens_kode
)::integer
FROM lineage l
JOIN lineage_scheme scheme ON scheme.slaegt_id=l.slaegt_id
  AND scheme.source_id IS NULL AND scheme.kind='presensliste'
WHERE l.presens_kode IS NOT NULL
ON CONFLICT(scheme_id,code) DO NOTHING;
INSERT INTO lineage_scheme_entry_lineage(entry_id,lineage_id,relation_kind)
SELECT entry.id,l.id,'canonical'
FROM lineage l
JOIN lineage_scheme scheme ON scheme.slaegt_id=l.slaegt_id
  AND scheme.source_id IS NULL AND scheme.kind='presensliste'
JOIN lineage_scheme_entry entry ON entry.scheme_id=scheme.id AND entry.code=l.presens_kode
WHERE l.presens_kode IS NOT NULL
ON CONFLICT DO NOTHING;

CREATE OR REPLACE VIEW person_source_coordinate_legacy
WITH (security_invoker=true) AS
...
CREATE TABLE IF NOT EXISTS private.source_record_placement (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  source_record_id uuid NOT NULL REFERENCES private.source_record(id),
  ...
  generation_label_raw text, kuld_label text,
...
CREATE UNIQUE INDEX IF NOT EXISTS source_record_placement_active_uidx
  ON private.source_record_placement(source_record_id,scheme_entry_id)
  WHERE status<>'superseded';
CREATE TABLE IF NOT EXISTS private.source_persona_placement (
  source_persona_id uuid NOT NULL REFERENCES private.source_persona(id),
...
CREATE OR REPLACE FUNCTION private.validate_source_record_placement()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,public,private AS $$
BEGIN
  IF NOT EXISTS(SELECT 1 FROM private.source_observation o
    ...
    WHERE o.id=NEW.header_observation_id AND o.observation_kind='header'
      AND anchor.source_record_id=NEW.source_record_id
      AND anchor.decision_status='accepted') THEN
    RAISE EXCEPTION 'EVIDENCE_PLACEMENT_HEADER_REQUIRED';
  END IF;
  IF EXISTS(SELECT 1 FROM private.source_record record
    ...
    WHERE record.id=NEW.source_record_id AND scheme.source_id IS NOT NULL
      AND scheme.source_id<>record.source_id) THEN
    RAISE EXCEPTION 'EVIDENCE_PLACEMENT_SOURCE_MISMATCH';
  END IF;
  RETURN NEW;
END $$;
CREATE OR REPLACE FUNCTION private.validate_source_persona_placement()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,public,private AS $$
BEGIN
  IF NOT EXISTS(SELECT 1 FROM private.source_persona persona
    ...
    WHERE persona.id=NEW.source_persona_id AND persona.source_id=record.source_id) THEN
    RAISE EXCEPTION 'EVIDENCE_PERSONA_PLACEMENT_SOURCE_MISMATCH';
  END IF;
  IF NOT EXISTS(SELECT 1 FROM private.source_observation observation
...
DO $$
BEGIN
  DROP TRIGGER IF EXISTS trg_source_record_placement_validate ON private.source_record_placement;
  CREATE TRIGGER trg_source_record_placement_validate BEFORE INSERT ON private.source_record_placement
    FOR EACH ROW EXECUTE FUNCTION private.validate_source_record_placement();
  DROP TRIGGER IF EXISTS trg_source_persona_placement_validate ON private.source_persona_placement;
  CREATE TRIGGER trg_source_persona_placement_validate BEFORE INSERT ON private.source_persona_placement
    FOR EACH ROW EXECUTE FUNCTION private.validate_source_persona_placement();
  DROP TRIGGER IF EXISTS trg_source_record_placement_append_only ON private.source_record_placement;
  CREATE TRIGGER trg_source_record_placement_append_only BEFORE UPDATE OR DELETE ON private.source_record_placement
    FOR EACH ROW EXECUTE FUNCTION private.reject_evidence_mutation();
  DROP TRIGGER IF EXISTS trg_source_persona_placement_append_only ON private.source_persona_placement;
  CREATE TRIGGER trg_source_persona_placement_append_only BEFORE UPDATE OR DELETE ON private.source_persona_placement
    FOR EACH ROW EXECUTE FUNCTION private.reject_evidence_mutation();
END $$;
...
-- ---- 2026-08-03: generelle beskrivelser og heraldik (Task 8) ----
CREATE TABLE IF NOT EXISTS entity_description (
  id            bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  subjekt_type  text NOT NULL,
  subjekt_id    bigint NOT NULL,
  kind          text NOT NULL CHECK (kind IN ('overview','history','caption','blazon_explanation')),
  language_code text NOT NULL DEFAULT 'da' CHECK (btrim(language_code)<>''),
...
CREATE TABLE IF NOT EXISTS private.entity_description_source (
...
CREATE OR REPLACE FUNCTION validate_entity_description_subject()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog,public AS $$
BEGIN
  IF (NEW.subjekt_type='person' AND EXISTS (SELECT 1 FROM public.person WHERE id=NEW.subjekt_id))
     OR (NEW.subjekt_type='slaegt' AND EXISTS (SELECT 1 FROM public.slaegt WHERE id=NEW.subjekt_id))
     OR (NEW.subjekt_type='lineage' AND EXISTS (SELECT 1 FROM public.lineage WHERE id=NEW.subjekt_id))
     OR (NEW.subjekt_type='coat_of_arms' AND EXISTS (SELECT 1 FROM public.coat_of_arms WHERE id=NEW.subjekt_id))
     OR (NEW.subjekt_type='media' AND EXISTS (SELECT 1 FROM public.media WHERE id=NEW.subjekt_id))
     OR (NEW.subjekt_type='estate' AND EXISTS (SELECT 1 FROM public.estate WHERE id=NEW.subjekt_id)) THEN
    RETURN NEW;
  END IF;
  RAISE EXCEPTION 'ENTITY_DESCRIPTION_SUBJECT_INVALID';
END $$;
DROP TRIGGER IF EXISTS trg_entity_description_subject ON entity_description;
CREATE TRIGGER trg_entity_description_subject BEFORE INSERT OR UPDATE OF subjekt_type,subjekt_id ON entity_description
  FOR EACH ROW EXECUTE FUNCTION validate_entity_description_subject();
INSERT INTO version_pk_registry(tabel,pk_cols,skip_cols) VALUES ('entity_description',ARRAY['id'],'{}')
ON CONFLICT(tabel) DO UPDATE SET pk_cols=excluded.pk_cols,skip_cols=excluded.skip_cols;
DROP TRIGGER IF EXISTS trg_log_entity_description ON entity_description;
CREATE TRIGGER trg_log_entity_description AFTER INSERT OR UPDATE OR DELETE ON entity_description
  FOR EACH ROW EXECUTE FUNCTION log_change();
ALTER TABLE entity_description ENABLE ROW LEVEL SECURITY;
