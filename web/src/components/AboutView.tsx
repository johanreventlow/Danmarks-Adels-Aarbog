...
export function AboutView({ about, personCount, estateCount, onPick }: { about: AboutSection[] | null; personCount: number; estateCount: number | null; onPick: (id: string) => void }) {
  return (
    <div style={{ padding: '30px 40px 50px', maxWidth: 680 }}>
      ...
      <div style={{ fontFamily: T.mono, fontSize: 11, letterSpacing: '.1em', textTransform: 'uppercase', color: T.muted2, marginTop: 8 }}>Indledning til stamtavlen · Danmarks Adels Aarbog</div>
      ...
      {!about ? <div style={{ color: T.muted3 }}>Henter…</div> : about.length ? about.map((s, i) => (
        <div key={i} style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontFamily: T.serif, fontSize: 17, lineHeight: 1.6, color: '#3d382f' }}>
            <NarrativRenderer tekst={s.tekst} onPickPerson={onPick} linkColor={T.bordeaux} inactiveColor={T.muted2} />
          </div>
        </div>
      )) : (
        <div style={{ border: '1px dashed rgba(34,31,26,.2)', borderRadius: 11, padding: 16, background: T.paper, fontSize: 14, lineHeight: 1.5, color: T.muted }}>
