...
export function FeedStyring({ role, model, run }: {
  ...
  const pill = (active: boolean) => ({ border: 0, borderRadius: 6, padding: '5px 8px', cursor: 'pointer',
    background: active ? T.bordeaux : T.beige, color: active ? T.paper : T.muted, fontFamily: T.mono, fontSize: 9 } as const);
  ...
  return <div style={{ maxWidth: 1080, margin: '0 auto', padding: '24px 28px 60px', color: T.ink }}>
    ...
    <div style={{ fontFamily: T.mono, fontSize: 9, color: T.muted2, marginTop: 3 }}>
    ...
    <div style={{ display: 'grid', gap: 7 }}>
      {pins.map((pin) => {
        ...
        return <div key={pin.kort_noegle} style={{ ...box, display: 'flex', alignItems: 'center', gap: 10 }}>
          <code style={{ fontFamily: T.mono, fontSize: 11 }}>{pin.kort_noegle}</code>
          <span style={{ ...pill(pin.handling === 'pin'), cursor: 'default' }}>{pin.handling}</span>
          <span style={{ fontFamily: T.mono, fontSize: 9, color: T.muted2 }}>{pin.oprettet_naar ?? 'tid ukendt'}</span>
          {status !== 'ok' && <span style={{ fontFamily: T.mono, fontSize: 9, color: status === 'dinglende' ? T.bordeaux : T.muted2 }}>{status}</span>}
          <span style={{ flex: 1 }} />
    ...
    <div style={{ display: 'grid', gap: 7 }}>
      {stories.map((story) => <div key={story.id} style={box}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <span style={{ color: T.muted, fontSize: 11 }}>{model?.byId[String(story.subjektId)]?.name ?? `Person #${story.subjektId}`}</span>
          <span style={{ flex: 1 }} />
        ...
        <div style={{ color: T.muted, fontSize: 12.5, marginTop: 4 }}>{story.tekst.slice(0, 180)}{story.tekst.length > 180 ? '…' : ''}</div>
        <div style={{ fontFamily: T.mono, color: T.muted2, fontSize: 8.5, marginTop: 5 }}>{story.publiceretDato ?? 'publiceringsdato ukendt'}</div>
      </div>)}
    ...
    <div style={box}>
      ...
      <div style={{ fontFamily: T.mono, fontSize: 9, color: T.gold, marginTop: 13 }}>ELLER STORY</div>
