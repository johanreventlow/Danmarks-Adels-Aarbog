  ...
  return (
    <>
      <div style={{ display: 'flex', overflowX: 'auto', scrollSnapType: 'x mandatory', gap: 8, overscrollBehaviorInline: 'contain', marginTop: 14 }}>
        {media.map((item, index) => {
          ...
          return (
            ...
            >
              <img src={item.mediumUrl} alt={label} style={{ width: '100%', height: 240, maxHeight: '42vh', objectFit: 'contain', display: 'block' }} />
            </button>
          );
        })}
      </div>
      {openIndex !== null ? <div onClick={(event) => event.stopPropagation()}>
        <Lightbox
          items={media.map((item) => ({ id: item.id, url: item.largeUrl, titel: item.titel, kunstner: item.kunstner, datering: item.datering }))}
          index={openIndex}
