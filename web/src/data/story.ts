// Web-spejl af mobilens tolerante story-loader. Netværksfejl og umigreret base
// betyder blot, at historie-kortene udelades.
...
export async function fetchStoryRows(): Promise<StoryRowsResult> {
  ...
  try {
    ...
    const kilder = await fetchInChunks<StoryKildeRow, string | number>(storyIds, (idsChunk) =>
      supabase.from('story_kilde').select('id,story_id,source_id,side')
    ...
    const sources = sourceIds.length > 0
      ? await fetchInChunks<StorySourceRow, string | number>(sourceIds, (idsChunk) =>
          supabase.from('source').select('id,udgave').in('id', idsChunk).order('id'),
        )
      : [];
...
export async function loadStorieBy(
  ...
  const result = await fetchStoryRows();
