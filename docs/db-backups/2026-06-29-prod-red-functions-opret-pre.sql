-- Pre-deploy backup: alle red_*-funktioner i prod INDEN red_opret_ny_entitet-migration
-- Dato: 2026-06-29 (migration 2026-06-30 morgen)
-- Query: SELECT p.proname, pg_get_functiondef(p.oid) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname LIKE 'red_%' ORDER BY 1;
...
CREATE OR REPLACE FUNCTION public.red_opret_union(p_partner_a bigint, p_partner_b bigint, p_type text, p_ordinal integer DEFAULT NULL::integer)
 RETURNS bigint
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.red_set_familie_konfidens(p_family_id bigint, p_person_id bigint, p_rolle text, p_konfidens text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.red_slet_familie_link(p_family_id bigint, p_person_id bigint, p_rolle text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.red_tilfoej_barn(p_family_id bigint, p_barn_id bigint, p_rolle text DEFAULT 'barn'::text, p_konfidens text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
