...
| `red_set_story_status` | `(p_story_id bigint, p_status text)` | Validerer mod de fire koder; **hver overgang TIL `'publiceret'` sætter `publiceret_dato := current_date`**; overgange væk fra publiceret rører den ikke (historisk dato bevares — klienten viser alligevel kun `status='publiceret'`, så en bevaret dato lækker intet) |
