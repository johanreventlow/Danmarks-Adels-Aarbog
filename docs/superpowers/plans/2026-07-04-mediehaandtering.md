...
- **Redaktør-upload — GJORT (Slice 0g, 2026-07-05):** ny `Change`-art `uploadMedia` → `red_upload_media`, følger `Change`→`buildRpcCall`→`submitChange`-mønsteret med dry-run/LIVE via `SkrivePreviewSheet` (bytes uploades KUN i LIVE). `expo-image-picker` + `expo-file-system` installeret (SDK-56-docs læst først, `File`-klassens `.bytes()` brugt, ikke deprecated `readAsStringAsync`). `app.json`-plugin med foto/kamera-permissions. Ny `mobile/src/lib/mediaUpload.ts` (pickImage/readFileBytes/buildStoragePath/performUpload) + "Materiale"-sektion/`MediaUploadSheet` i `redaktion/person/[id].tsx`. `/simplify`-renset (4 fund rettet). tsc+269/269 jest+lint grønne. Porteret til web samme dag (`web/src/Redaktion.tsx`, browser-native `<input type=file>`-upload).
- **Slet/afkobl + objekt-foto — GJORT (Slice 0h, 2026-07-05):** brugerens egen prod-test af Slice 0g fandt en reel bug (uploads sad fast som `kladde` — `submitChange` kaldte aldrig `red_bekraeft_media_upload`), rettet på begge platforme. Tilføjede derefter "Fjern" (afkobl, genbruger `red_slet_relation` uændret) og "Slet" (nyt `red_fjern_media`-RPC, blødt fjern via `upload_status='fjernet'` — gratis fortrydbar via eksisterende versionering, ingen ny RLS-politik, rører aldrig Storage). Objekt-foto (gods/våben) implementeret på begge platforme via `red_upload_media`s allerede-eksisterende `p_objekt_type`-gren; web via delt `renderMateriale()` i `renderGenericEditor()`, mobile via en bevidst minimal `entitet/materiale.tsx`-skærm (ingen generisk detail-editor findes endnu på mobile). `/simplify` fandt + rettede 4 reelle fund inkl. et datahazard i `red_bekraeft_media_upload` (hærdet med en guard). tsc+272/272 jest (mobile) + tsc+155/155 vitest+build (web) grønne. **Runtime-verifikation af selve slet/afkobl/objekt-foto-UI'en udestår** (samme kendte gap: intet browser-driver/simulator-tap-værktøj i repo'et).
- **Entitet-browser** (`redaktion/entitet/[type].tsx`) viser `medieListe` read-only (uændret) — gods/våben-rækker er nu tappbare til materiale (Slice 0h), øvrige typer stadig uden detail-editor.
