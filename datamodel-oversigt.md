...
- **Slægtslinjer / grene** — fx de forskellige Reventlow-linjer (svarer til TNG's "branches"). *Implementeret trin (a): `lineage`-entitet `(id, source_id, kode, navn)` navngiver linjerne (I=Den holstenske linje … V=Den grevelige linje af 1673). Trin (b) — adling→ny slægt, forgrening, eget våben, person↔linje m. konfidens — udestår additivt; se `docs/decisions.md`.*
